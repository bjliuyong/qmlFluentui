...
Item {
    ...
    readonly property int kStatusBarMinH: 30

    readonly property int staticMinHeight: (kMargin * 2) + kSpacing * 2 + kTopMinH + kBottomMinH + kStatusBarMinH
    readonly property int staticMinWidth: (kMargin * 2) + kBottomMinW

    implicitWidth: staticMinWidth
    implicitHeight: staticMinHeight

    property int topWeight: 2
    property int bottomWeight: 18
    property int statusBarWeight: 1
    property int tabHeaderHeight: 36

    property real _pixelPerRatioH: Math.max(0, baseRoot.height) / (topWeight + bottomWeight + statusBarWeight)
    ...
    property alias statusBarContent: statusBar.data
    ...
    ColumnLayout {
        ...
        FluShadowGroupBox {
            Layout.fillWidth: true
            Layout.fillHeight: false
            Layout.preferredHeight: statusBarWeight * _pixelPerRatioH
            Layout.minimumHeight: kStatusBarMinH
            title: ""
            Item { id: statusBar; anchors.fill: parent; anchors.margins: 10; clip: true }
        }
    }
}
